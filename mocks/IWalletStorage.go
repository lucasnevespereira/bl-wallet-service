...
// UpdateBalance provides a mock function with given fields: wallet, transaction
func (_m *IWalletStorage) UpdateBalance(wallet *storage.RowWallet, transaction *models.TransactionRequest) error {
	ret := _m.Called(wallet, transaction)

	var r0 error
	if rf, ok := ret.Get(0).(func(*storage.RowWallet, *models.TransactionRequest) error); ok {
		r0 = rf(wallet, transaction)
	} else {
		r0 = ret.Error(0)
	}
